 ...
 */
const getNodeModuleVersion = (npmDir, nodeModuleName) => (
...
const getNpmDepFiles = (npmDir, depPaths) => (
  depPaths.map((depPath) => {
    ...
    return {
      ...
      nodeModuleVersion: getNodeModuleVersion(npmDir, nodeModuleName), // e.g. 0.4.11
 ...
 */
const retrieveNestedDeps = (npmDir, inputFiles, existingDepPaths) => {
  ...
  return [
    ...nestedDepFiles,
    ...retrieveNestedDeps(npmDir, nestedDepFiles, updatedExistingDepPaths),
  ];
};
...
const retrieveDepFiles = ({ depPaths, npmDir, ethpmDir }) => {
  ...
  if (npmDir) {
    ...
    npmDepFiles = [...npmDepFiles, ...retrieveNestedDeps(npmDir, npmDepFiles, depPaths)];
  }

  // retrieve EthPM dependencies
  if (ethpmDir) {
    ethpmDepFiles = getNpmDepFiles(ethpmDir, depPaths);
    // recursively retrieve the dependencies of the depFiles
    ethpmDepFiles = [...ethpmDepFiles, ...retrieveNestedDeps(ethpmDir, ethpmDepFiles, depPaths)];
  }

  // create one big list of all file objects
  return [...npmDepFiles, ...ethpmDepFiles];
};
