...
/**
 * turn filepath into EthPM path, which has contracts/ between module and dir/file name
 *
 * @param {string} filePath - the file path, e.g. some-module/some-dir/some-file.sol
 * @return {string} path converted to actual EthPM path, e.g. some-module/contracts/some-dir/some-file.sol
 */
const turnIntoEthpmPath = filePath => (
  filePath.replace(/([a-zA-Z_\-.]+)\//, '$1/contracts/')
);
...
const getParentFiles = (rootFile, allFiles) => {
  ...
  const depFiles = rootFile.dependencies
    .map(dep => ({ ...dep, path: crossPlatformPath(dep.path) }))
    .map(dep => allFiles.find(file => crossPlatformPath(dep.path) === (
      file.type === 'EthPM' ? turnIntoEthpmPath(file.path) : file.path
    )))
    .reduce((memo, depFile) => (
      [...memo, ...getParentFiles(depFile, allFiles)]
    ), []);

  return [...depFiles, rootFile];
};
...
const getHighestDepPos = (sortedFiles, targetFileDepPaths) => (
  sortedFiles.reduce((highest, sortedFile, idx) => (
    targetFileDepPaths.some((depPath) => {
      const targetPath = sortedFile.type === 'EthPM'
        ? turnIntoEthpmPath(sortedFile.path)
        : sortedFile.path;
...
const combineFiles = files => (
  [
    ...
    generateDepModuleVersionComments(files),
 ...
 */
const combine = ({ srcDir, npmDir, ethpmDir, destDir }) => { // eslint-disable-line object-curly-newline, max-len
  // get all files in the provided source directory and all its subdirectories
  const contractFiles = getContractFiles(srcDir, undefined, { ethpmDir, destDir });
  ...
  // get a list of file paths of non-relative dependencies found in the contract files
  ...
  // write a File.sol file for each of the contract files to the destDir
  ...
  !process.env.TESTING && console.log(`SUCCESS wrote combine output to ${destDir}`); // eslint-disable-line
};
