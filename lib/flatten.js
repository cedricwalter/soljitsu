...
const insertNodeModuleVersion = file => (
  [
    ...
    generateDepModuleVersionComments([file]),
    file.content.split('\n').slice(1).join('\n'),
  ]
    .join('\n')
);
  ...
  targetFile.content
    ...
    .map((line) => {
      if (line.startsWith('import')) {
        ...
        // get the file item
        const fileItem = allFiles.find(file => (
          crossPlatformPath(file.path ===
            // EthPM files are always located inside a directory contracts/
            // this however is not included in the path of the require call, so
            // remove contracts/ from the path
            (targetFile.type === 'EthPM'
              ? importPath.replace(/\/contracts/, '')
              : importPath))
        ));
...
const flatten = ({ srcDir, npmDir, ethpmDir, destDir }) => { // eslint-disable-line object-curly-newline, max-len
  ...
  !process.env.TESTING && console.log(`SUCCESS wrote flatten output to ${destDir}`); // eslint-disable-line
};
